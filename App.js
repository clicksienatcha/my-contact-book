...
export default class App extends React.Component {
  ...
  async componentDidMount(){
    ...
    let response = await fetch("https://randomuser.me/api/?results=20");//เรียก json มา
    ...
    this.setState({
      items:json.results,//หาก API ไม่มีคำ Data มาไม่ต้องใส่ results
          ...
          <List dataArray={this.state.items} 
                renderRow={(rowData)=>{
                  return (
                    ...
                    <Left>
                      <Thumbnail source={{uri:rowData.picture.large}} />
                    </Left>
                    <Body>
                      <Text>{rowData.name.frist} {rowData.name.last}</Text>
